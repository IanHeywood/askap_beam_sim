...
def askap_beam(E,lm,p):
  ...
  if use_ideal:
    ns.maj_x << 1.0 * ref_freq / Meq.Freq()
    ns.min_x << 1.0 * ref_freq / Meq.Freq()
    PA_x = 0.0
    A_x = 1.0
  else:
    ns.maj_x << beam_params_xx[2] * ref_freq / Meq.Freq()
    ns.min_x << beam_params_xx[3] * ref_freq / Meq.Freq()
  ...
  if use_ideal:
    ns.maj_y << 1.0 * ref_freq / Meq.Freq()
    ns.min_y << 1.0 * ref_freq / Meq.Freq()
    PA_y = 0.0
    A_y = 1.0
  else:
    ns.maj_y << beam_params_yy[2] * ref_freq / Meq.Freq()
    ns.min_y << beam_params_yy[3] * ref_freq / Meq.Freq()
  ...
  # a_x = (Meq.Pow(ns.l_x,2))*((((math.cos(PA_x*DEG))**2.0)/(2.0*(maj_x**2.0))) + (((math.sin(PA_x*DEG))**2.0)/(2.0*(min_x**2.0))))
  # b_x = 2.0*Meq.Multiply(ns.l_x,ns.m_x)*(((math.sin(2.0*PA_x*DEG))/(4.0*(maj_x**2.0))) - ((math.sin(2.0*PA_x*DEG))/(4.0*(min_x**2.0))))
  # c_x = (Meq.Pow(ns.m_x,2))*((((math.sin(PA_x*DEG))**2.0)/(2.0*(maj_x**2.0))) + (((math.cos(PA_x*DEG))**2.0)/(2.0*(min_x**2.0))))

  # a_y = (Meq.Pow(ns.l_y,2))*((((math.cos(PA_y*DEG))**2.0)/(2.0*(maj_y**2.0))) + (((math.sin(PA_y*DEG))**2.0)/(2.0*(min_y**2.0))))
  # b_y = 2.0*Meq.Multiply(ns.l_y,ns.m_y)*(((math.sin(2.0*PA_y*DEG))/(4.0*(maj_y**2.0))) - ((math.sin(2.0*PA_y*DEG))/(4.0*(min_y**2.0))))
  # c_y = (Meq.Pow(ns.m_y,2))*((((math.sin(PA_y*DEG))**2.0)/(2.0*(maj_y**2.0))) + (((math.cos(PA_y*DEG))**2.0)/(2.0*(min_y**2.0))))
...
_askap_option_menu = TDLCompileMenu('Select ASKAP beam',
  ...
  TDLMenu('Use ideal beam',toggle="use_ideal",doc="Click this to use circular Gaussians"))
